...
\fi
The need to reduce the energy consumption for power-loss protection arises in
different contexts. In a few studies, the total energy consumption was reduced
\EUNJI{past? present?}
by speeding up the backup process upon power failure using fast
media~\cite{GuoYZC13date,HuangWQLS15hpcc}.  Guo et al.~\cite{GuoYZC13date}
reduce the capacitance requirement by writing back the volatile buffer data
into Phase Change Random Access Memory (PRAM), which is faster and uses less 
power than NAND flash does. 
% They argue that this reduction
...
% of storage device.  As a similar approach, 
Smartbackup~\cite{HuangWQLS15hpcc}
proposes dynamic NAND channel allocation and single-level cell (SLC) mode
programs to make the dump process shorter at sudden power-off. 
\iffalse
It makes full
...
using a small size of capacitor. However, DRWB only regards
...
\fi
Another approach to reducing the capacitor size is achieved by only protecting
a small portion of the volatile buffer. A Dual-Region Write
Buffer (DRWB)~\cite{KimK15sac} divides the internal-SSD buffer into a small protected
region and a large unprotected region. When the data on the unprotected region
is updated, DRWB logs the delta for the page in the protected region. However,
DRWB only performs this for the user data, having no consideration for the
metadata such as mapping tables, despite that it actually accounts for most of
the internal buffer of SSDs.  \iffalse
Furthermore, commercial SSDs typically do not cache read data in the buffer because the host memory can serve as a cache memory of the storage device. For these reasons, the effectiveness of DRWB may be limited in a practical environment.
\fi
...
sufficient capacitance during the SSD lifetime, which is like a commercial SSD.
\fi

Prior work by Chen et. al~\cite{ChenLZ19tc} has also addressed the challenge of
capacitance constraints for scalable SSDs. They observed that a small
write buffer can be effective for reducing write traffic in particular
applications that perform heavy journaling. Motivated by this observation,
they present an application-SSD co-design to reduce the data writes buffered
for heavy journaling applications. However, their approach is only applicable
to systems that perform journaling and require changes to the application code. 
...
% scalable SSDs.
There are a few studies that raised an issue about capacitance constraints in scalable SSDs.  
Chen et al. project that even the high capacity of SSDs will use
...
for heavy journaling applications. 

They propose to protect write-hot
...
% It reduced substantial amount of flash memory write traffic with few megabytes of capacitor-powered write buffer, but 
Despite a large reduction in write traffic, it 
\fi
% However, they inherently have a weakness that the proposed scheme is specific to heavy journal applications and requires change of application code.
%to benefit from its scheme.

SpartanSSD~\cite{lee2021spartanssd}, which is most related to this work,
...
small DRAM and flash memory. This hybrid journal is highly flexible in
terms of capacity; thus, it enables a timely checkpoint that reflects the log
...
it increases the recovery time, which could be extremely harmful
