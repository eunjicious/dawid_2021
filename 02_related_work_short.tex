...
\fi
The need to reduce the energy consumption for power-loss protection
arises in different contexts. A few studies reduce the total energy consumption
by speeding up the backup process upon power failure using fast media~\cite{GuoYZC13date,HuangWQLS15hpcc}. 
Guo et al.~\cite{GuoYZC13date} reduce the capacitance requirement by writing back the volatile buffer
data into PRAM (Phase Change Random Access Memory), which is faster and uses
lower power than NAND flash. 
...
terms of capacity, and thus, it enables a timely checkpoint that reflects the log
